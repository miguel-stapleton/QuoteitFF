...
import { agneFlatRate, OVERNIGHT_TRAVEL_THRESHOLD, OVERNIGHT_TRAVEL_FEE, getMinimumGuests } from '../data/services';

// Index of the main wedding day for minimum-guest billing purposes.
// Day 0 is always treated as the main day; change here if that ever shifts.
const MAIN_WEDDING_DAY_INDEX = 0;

// Returns effective travel fee and overnight flag for each day index.
// Within a run of calendar-adjacent days all with travelFee > THRESHOLD, the first day
// keeps its full fee and every subsequent day is charged the flat OVERNIGHT_TRAVEL_FEE.
function computeEffectiveTravelFees(
  days: Array<{ travelFee: number }>,
  dates: string[]
): Array<{ effectiveFee: number; isOvernightRate: boolean }> {
  const result = dates.map((_, i) => ({
    effectiveFee: days[i]?.travelFee ?? 0,
    isOvernightRate: false
  }));

  const areDatesAdjacent = (a: string, b: string) =>
    Math.abs(new Date(b).getTime() - new Date(a).getTime()) === 86_400_000;

  let i = 0;
  while (i < days.length) {
    if ((days[i]?.travelFee ?? 0) > OVERNIGHT_TRAVEL_THRESHOLD) {
      // Extend run while next day also qualifies and dates are consecutive
      let j = i + 1;
      while (
        j < days.length &&
        (days[j]?.travelFee ?? 0) > OVERNIGHT_TRAVEL_THRESHOLD &&
        areDatesAdjacent(dates[j - 1], dates[j])
      ) {
        j++;
      }
      // Apply overnight rate to every day after the first in this run
      for (let k = i + 1; k < j; k++) {
        result[k].effectiveFee = OVERNIGHT_TRAVEL_FEE;
        result[k].isOvernightRate = true;
      }
      i = j;
    } else {
      i++;
...
export function calculateQuote(input: CalculationInput): {
  ...
  const grandSummary = calculateGrandSummary(calculations, input.ivaEnabled, input.ivaRate);
...
function calculateMakeupService(
  ...
  // Compute effective travel fees accounting for overnight rule (§3a)
  const effectiveTravelFees = computeEffectiveTravelFees(
    form.perDay || [],
    weddingDates
  );

  // Full travel fee = max raw fee across days (used for band calculation, §6a)
  const fullTravelFee = Math.max(0, ...(form.perDay || []).map(d => d.travelFee || 0));
  const minGuests = getMinimumGuests('makeup', form.artist, fullTravelFee);
  // Guests on day 0 — used to decide warn vs. bill
  const mainDayGuests = form.perDay?.[MAIN_WEDDING_DAY_INDEX]?.guests ?? 0;
  const minGuestsPriorityWarnings: PriorityWarning[] = [];
  // If guests were specified and a minimum applies, queue a priority warning
  if (minGuests > 0 && mainDayGuests > 0) {
    minGuestsPriorityWarnings.push({
      id: `auto_min_guests_makeup_${Date.now()}`,
      text: `For this location, due to its distance, this artist requires the minimum of ${minGuests} guests under their care (not including the bride), or equivalent payment.`,
      autoGenerated: true
    });
  }
  ...
  const dayBreakdowns: DayBreakdown[] = weddingDates.map((date, idx) => {
    ...
    const { effectiveFee, isOvernightRate } = effectiveTravelFees[idx];

    const lines: CalculationLine[] = [];

    // Guests — on main day, bill minimum if guests unspecified and minimum applies
    if (idx === MAIN_WEDDING_DAY_INDEX && minGuests > 0 && mainDayGuests === 0) {
      lines.push({
        label: 'Guests (minimum number of guests for this location)',
        qty: minGuests,
        unit: makeupPrices.guestUnit,
        total: minGuests * makeupPrices.guestUnit
      });
    } else if (day.guests > 0) {
      lines.push({
        label: 'Guests',
        qty: day.guests,
        unit: makeupPrices.guestUnit,
    ...
    // Travel fees (car-based model) per day, using effective fee
    if (effectiveFee > 0) {
      ...
      const travellingCarsTotal = effectiveFee * carCount;
      if (travellingCarsTotal > 0) {
        const destination = day.beautyVenue ? ` to ${day.beautyVenue}` : '';
        const overnight = isOvernightRate ? ' — overnight rate' : '';
        lines.push({
          label: `Travelling fee (cars)${destination}${overnight}`,
          qty: carCount,
          unit: effectiveFee,
      ...
      // 2) Assistant travel fee = 35% × effectiveFee × assistants
      const assistantUnit = 0.35 * effectiveFee;
    ...
    return { date, lines, subtotal, venue: day.beautyVenue || undefined, label: day.dayLabel || undefined };
  });
  ...
  return {
    ...
    dayBreakdowns,
    priorityWarnings: minGuestsPriorityWarnings
  };
}
...
function calculateHairService(
  ...
  // Full travel fee = max raw fee across days (used for band calculation, §6a)
  const fullTravelFee = Math.max(0, ...(form.perDay || []).map(d => d.travelFee || 0));
  const minGuests = getMinimumGuests('hair', form.artist, fullTravelFee);
  const mainDayGuests = form.perDay?.[MAIN_WEDDING_DAY_INDEX]?.guests ?? 0;
  const minGuestsPriorityWarnings: PriorityWarning[] = [];
  if (minGuests > 0 && mainDayGuests > 0) {
    minGuestsPriorityWarnings.push({
      id: `auto_min_guests_hair_${Date.now()}`,
      text: `For this location, due to its distance, this artist requires the minimum of ${minGuests} guests under their care (not including the bride), or equivalent payment.`,
      autoGenerated: true
    });
  }
  ...
  const dayBreakdowns: DayBreakdown[] = weddingDates.map((date, idx) => {
    ...
    if (idx === MAIN_WEDDING_DAY_INDEX && minGuests > 0 && mainDayGuests === 0) {
      lines.push({
        ...
        total: minGuests * hairPrices.guestUnit
      });
    } else if (day.guests > 0) {
      lines.push({
        label: 'Guests',
...
function calculateAgneFlatRate(
  ...
  // Full travel fee = max raw fee across days (used for band calculation, §6a)
  const fullTravelFee = Math.max(0, ...(form.perDay || []).map(d => d.travelFee || 0));
  const minGuests = getMinimumGuests('hair', HairArtist.Agne, fullTravelFee);
  const mainDayGuests = form.perDay?.[MAIN_WEDDING_DAY_INDEX]?.guests ?? 0;
  const minGuestsPriorityWarnings: PriorityWarning[] = [];
  if (minGuests > 0 && mainDayGuests > 0) {
    minGuestsPriorityWarnings.push({
      id: `auto_min_guests_agne_${Date.now()}`,
      text: `For this location, due to its distance, this artist requires the minimum of ${minGuests} guests under their care (not including the bride), or equivalent payment.`,
      autoGenerated: true
    });
  }
  ...
  weddingDates.forEach((date, idx) => {
    ...
    if (idx === 0) {
      ...
      // Guests on Day 1: Agne includes 3; only bill extras (or minimum if unspecified)
      const effectiveGuests = (minGuests > 0 && mainDayGuests === 0) ? minGuests : day.guests;
      const effectiveGuestLabel = (minGuests > 0 && mainDayGuests === 0)
        ? 'Extra Guests — minimum for this location (beyond 3 included)'
        : 'Extra Guests (beyond 3 included)';
      if (effectiveGuests > 3) {
        const extraGuests = effectiveGuests - 3;
        lines.push({
    ...
    // Travel fees (same logic for all days), using effective fee
    if (effectiveFee > 0) {
      const numPeople = Math.max(1, (day as any).numPeople || 1);
      const numCars = Math.max(0, day.numCars || 0);
      const carCount = Math.min(numCars, numPeople);
      ...
      const assistantUnit = agneFlatRate.travelFeeMultiplier * effectiveFee;
      const assistantTravelTotal = assistantUnit * assistantsCount;
      if (assistantTravelTotal > 0) {
        lines.push({
          label: 'Assistant travel fee',
    ...
    const subtotal = lines.reduce((s, l) => s + l.total, 0);
    dayBreakdowns.push({ date, lines, subtotal, venue: day.beautyVenue || undefined, label: day.dayLabel || undefined });
  });
...
function calculateGrandSummary(
  ...
  if (ivaEnabled && ivaRate) {
    const ivaAmount = grandTotal * ivaRate;
    return {
      grandTotal,
      totalPaid,
      ...
      ivaRate,
      ivaAmount,
      totalInclIva: grandTotal + ivaAmount
    };
  }

  return { grandTotal, totalPaid, totalDue: Math.max(0, totalDue) };
}
