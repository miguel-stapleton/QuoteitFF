...
export function calculateQuote(input: CalculationInput): {
  ...
  calculations.forEach(calc => {
    if (calc.serviceType === 'makeup') {
      ...
      const warnings = generatePriorityWarnings('makeup', calc.artistName, numPeople, calc.payments, calc.lines.find(line => line.label === 'Bridal MU')?.unit || 0, calc.lines.find(line => line.label === 'Guests')?.unit || 0);
      priorityWarnings.push(...warnings);
    } else if (calc.serviceType === 'hair') {
      const numPeople = Math.max(1, calc.dayBreakdowns.reduce((sum, day) => sum + day.lines.reduce((s, line) => s + (line.label === 'Guests' ? line.qty : 0), 0), 0));
      const warnings = generatePriorityWarnings('hair', calc.artistName, numPeople, calc.payments, calc.lines.find(line => line.label === 'Bridal H')?.unit || 0, calc.lines.find(line => line.label === 'Guests')?.unit || 0);
      priorityWarnings.push(...warnings);
    }
  });
...
function generatePriorityWarnings(
  ...
  if (numPeople > 1) {
    ...
    if (!hasAssistantDeposit) {
      ...
      const assistantLabel = numAssistants === 1 ? 'assistant' : 'assistants';
      warnings.push({
        id: `auto_assistant_${serviceType}_${Date.now()}`,
        text: `Assistant ${assistantLabel} Deposit(s): there ${numAssistants === 1 ? 'is' : 'are'} ${numAssistants} assistant ${serviceName}(s) described in your quote, but this artist will only be booked once a €${guestUnitPrice.toFixed(2)} deposit has been received.`,
        autoGenerated: true
      });
    }
  }
